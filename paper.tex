...
Sec.~\ref{ssec:test_equilibrium}.
For the \emph{spectral} method we use $J(\varepsilon)$ directly,
...
are badly characterized by the \emph{noscat} treatment.
...
Fig.~\ref{fig:avg_eps_collapse} from the formal solution explored in the
simple configurations above.
...
The relation is quite simple if we simplify our realistic model
to that of a homogeneous matter profile and
background scattering field.

In the envelope the local temperature is low
and the ratio $\chi^*_a/\chi_s$ small,
so $f_{\rm SE}$ strongly dominates over $f_{\rm AE}$.
By expanding the exponential of
Eqn.~\ref{eqn:fse_homogeneous_general} as we did for
Eqn.~\ref{eqn:fae_homogeneous_limits},
and furthermore factoring out the dominant ${\rm energy}^2$ dependence from the
scattering opacity $\chi_s\equiv\varepsilon^2 \zeta_s$, we have
\begin{equation}
...
Pinching parameters (calculated by eye) for the species
$\{\nu_e,\bar{\nu}_e,\nu_x\}$ are $\alpha\approx\{3.6,5.1,2.3\}$,
when we use the average energies from the \emph{spectral}
\emph{scat} method in Tab.~\ref{tab:nsns_rt_vs_m1}.
...
envelope to within 20\%
% From by-eye examinations of <eps>(cosA) plot using data from
% MCDATA=$DIR/ResultsSherwood-02
% RTDATA_GR=$DIR/ResultsBrett-B-03/Test-scat-01/
% RTDATA_SP=$DIR/ResultsBrett-A-03/Test-scat-01/
%          nue     nua     nux
of the Monte Carlo and the \emph{scat} ray tracing calculations,
even though we have made some drastic simplifications to our model.
...
was simulated using an M1 transport scheme for the neutrinos, evolving the
