...
This paper introduces the use of neutrino densities and fluxes evolved in a
truncated moment simulation to estimate the background field, $\Phi_{\rm SE}$.
...
\subsection{Homogeneous Equilibrium}
\label{ssec:test_equilibrium}
Homogeneous matter, optically thick. Plot in Fig.~\ref{fig:homogeneous_isotropic}.
\todo{describe test}

\subsection{Doppler Shift}
\label{ssec:test_doppler}
Homogeneous matter moving with v in +z direction. Stationary observer.
\todo{add plot of $\varepsilon(\mu)$}

\subsection{Gravitational Redshift and Geodesic Curvature}
...
\todo{run test for optically thick and thin}

\subsection{Massive Collapsed Star with Scattering Envelope}
\label{ssec:test_scat_envelope}
...
\todo{add plots of $\langle \varepsilon \rangle(\mu)$}
...
\begin{figure}
  ...
  \caption{Equilibrium distribution functions in a homogeneous background at
