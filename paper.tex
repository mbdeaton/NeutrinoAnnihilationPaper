...
decomposition via Eqn.~\ref{eqn:def_momentum}: the fluid, or comoving, observer,
$u^\alpha=dx^\alpha/ds$, and the Eulerian, or normal, observer,
$n_\alpha=-\alpha \,\partial_\alpha t$.
Here $ds$ is a differential of proper time, $t$ is coordinate time,
and $\alpha$ is lapse.
...
\item
  ...
  \begin{equation}
    ...
    \frac{\mathscr{F}_3(\mu_\nu/T)}{\mathscr{F}_2(\mu_\nu/T)},
  \end{equation}
  where $\mathscr{F}_j$ is the Fermi integral
  \begin{equation}
    \mathscr{F}_j(\eta) = \int_0^\infty dx \, x^j (1+e^{x-\eta})^{-1}.
  \end{equation}
\item
  ...
  \begin{align}
    J(\varepsilon) &=
    \frac{J}{\mathscr{F}_3(\mu_\nu/T)}
    (1+\exp(\varepsilon/T_\nu-\mu_\nu/T)^{-1}, \\
    H^\mu(\varepsilon) &=
    \frac{H^\mu}{\mathscr{F}_3(\mu_\nu/T)}
    (1+\exp(\varepsilon/T_\nu-\mu_\nu/T)^{-1}.
  \end{align}
\end{enumerate}
