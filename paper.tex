...
\footnote{TODO: clarify quasistationary timescale}
...
in the test presented in Sec.~\ref{ssec:test_collapse}
(a massive collapsed star following core bounce).
...
\begin{figure}
  ...
  \caption{Mean free paths
    ...
    $T=3.7$~MeV, $Y_e=0.12$).
    In the fluid rest frame these are given by the inverses of the opacities,
...
we also examine $\langle \tilde{\varepsilon}\rangle$ as a function of
viewing angle (via Eqn.~\ref{eqn:avg_eps_per_cosA}).
In the interior of the star
$\langle \tilde{\varepsilon_{\nu_x}}\rangle$ is constant across viewing angle,
\footnote{TODO: need to finish cleaning up this description}
and find the counterintuitive
feature of increasing average energies at the limb of the star
for $\nu_e$ and $\bar{\nu}_e$.
In contrast $\langle \tilde{\varepsilon} \rangle$
for $\nu_x$ is constant across angles, as expected when the star
is optically thin at even the highest energies $\tilde{\varepsilon}\sim100$~MeV.

For viewing angles $\cos A \sim 0.75$, at the limb of the star,
rays pass through significantly less material, i.e.\ $s \ll R$.
Because of the opacity's approximate $\varepsilon^2$ dependence,
the highest-energy rays saturate to $f^{\rm eq}$, increasing the average spectral energy.
...
\footnote{TODO: update MC results 02->03}
...
\footnote{TODO: way too-low angular resolution}
...
\footnote{TODO: get accurate M1 numbers from Francois}
...
\footnote{TODO: Need $R$ magnitude not just difference}
...
\footnote{TODO: also examine spectra or average energies}
...
The matter potential is always positive;
and outside the accretion disks formed in neutron star mergers,
in which the disrupted neutron-star matter is rapidly releptonizing,
the neutrino self-interaction potential is large and negative.
...
\footnote{TODO: improve resolution and update}
...
\footnote{TODO: note coordinate ray}
...
We see that the matter-neutrino resonance occurs in both treatments,
...
\end{equation}
with $n_{e^-}$, $n_{e^+}$, and $n_b$, the number densities of electrons
