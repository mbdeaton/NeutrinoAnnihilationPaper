...
\affiliation{Department of Astronomy and Oskar Klein Centre,
  Stockholm University, Alba Nova, SE-10691, Sweden}
\affiliation{Department of Physics,
  North Carolina State University, Raleigh, NC 27695, USA}
...
\begin{abstract}
  ...
  In that environment, we find qualitatively different fluxes, spectra,
  and oscillation behaviors when elastic scattering is included.
\end{abstract}
...
solutions to the Boltzmann Equation are not feasible.
...
Angular and spectral neutrino distributions in neutron star merger simulations
have just recently become available with a coupled Monte-Carlo-M1 scheme
\citep{fouc2017-m1_with_mc,fouc2018-mc_for_errors}.
In this work, however, we present a ray tracing method to compute neutrino
distribution functions from the more widely available state-of-the-art
general relativistic M1 transport hydrodynamics simulations.
...
or over a more general set, e.g. the set of all elastic scattering interactions.
...
it has only two remaining degrees of freedom; we make this explicit by defining its
...
\end{align}
where the spacetime metric components $\alpha$, $\beta^i$, and $g^{ij}$
are defined in App.~\ref{sec:definitions}.
...
from which we compute an error estimate by comparing the 3rd and 2nd order solutions.
After each step is taken, the error estimates $\Delta_i$ for each of the
nine variables of Eqns.~\ref{eqn:num_x}--\ref{eqn:num_fse} are used
to compute an overall error measure of the step, $D$, given by
\begin{equation}
  \label{eqn:err_measure}
  D = \max\left(\frac{\Delta_i}{A+R|V_i|}\right),
\end{equation}
where the max is taken over the set of all variables $V_i$
and their error estimates $\Delta_i$,
and $A$ and $R$ are the absolute and relative error tolerances.
If $D \geq 1$ the step size is decreased and the step recomputed;
if $D < 1$ the next step size is increased.
% The whole integration is run by OdeIntegrator::AdaptiveDense, which contains
%   some essential pieces: 1) OdeStepper calculates the change in the variables,
%   2) OdeErrorMeasure calculates a single double representing the overall error
%   of the step, and 3) OdeController either initiating a step redo or
%   calculating the next step size.
% See:
% * OdeIntegrators::AdaptiveDense
% * OdeErrorMeasures::ScaledAbsRel
% * OdeSteppers::RungeKutta3 and OdeSteppers::DormandPrince5
% * OdeControllers::ProportionalIntegral_Base::StepSizeToAttempt
...
for which the relative tolerances are set to $R=6\times10^{-4}$,
and the absolute tolerances are set to $A=6\times10^{-24}$.
...
and stored in pseudospectral representation \cite{duez2008-first_spec_bhns},
an efficient scheme for representing a continuous field with a finite set of data points.
...
an open-source neutrino interaction library,
and is identical to the LS180 opacity table used in the paper introducing
that tool \cite{ocon2015-gr1d_with_nu}.
...
\begin{figure}
  ...
  \caption{Mean free paths
    ...
    In these data they are exactly identical
...
$N_{\tilde{\varepsilon}}$ the number of energy samples,
...
the open source supernova evolution code \lstinline{GR1D}
...
pseudospectral grid composed of 11 spherical-shell subdomains
...
we turn off inelastic scattering where it is included (in the Monte Carlo code),
and we turn off general relativistic effects where they are included
(in the ray tracing and M1 codes).
...
\begin{equation}
  ...
  \varepsilon^{a-2}
  \exp\left(-(a+1)\frac{\varepsilon}{\langle\varepsilon\rangle}\right),
\end{equation}
...
\begin{align}
  ...
  \langle\tilde{\varepsilon}\rangle^{\rm sc}
  &\approx \frac{a+3}{a+1} \langle\varepsilon\rangle, \\
...
The errors in our moments introduced by this assumption
...
coordinate spheres at the observers' locations.
...
of all-sky luminosities, we expect some disagreement since the two
treatments differ fundamentally.
In addition to differences in transport methodologies,
the M1 fluxes are integrated over the outer boundary of the finite-difference grid
...
As the ray tracing and M1 measurements both show,
...
number density over incoming polar angle $\cos A$
for the observer on the polar axis $\theta=0^\circ$,
and in the equatorial plane $\theta=90^\circ$.
This number density is defined
\begin{align}
  \label{eqn:G_per_cosA_2}
  \tilde{G}(\cos A) &=
  \frac{\Delta}{(2\pi)^3} \sum\limits_{m=0}^{N-1}\tilde{\varepsilon}_m^2 f_m(\cos A),
\end{align}
with $\Delta \equiv \Delta\tilde{\varepsilon} \Delta B$,
$N \equiv N_{\tilde{\varepsilon}} N_B$,
$f_m(\cos A) \equiv f(\tilde{\varepsilon}_m,\cos A,B_m)$,
and $m$ the index labeling each ray at polar angle $\cos A$.
The integral of this quantity over $d\cos A$ gives
the total number density $\tilde{G}$, according to Eqn.~\ref{eqn:G}.
...
\emph{noscat} treatments for incoming angles $\cos A \gtrsim 0.95$
...
since the energy emission due to $e^{-}e^{+}$ annihilation,
...
\begin{figure}
  ...
  \caption{Distribution of number density $\tilde{G}(\cos A)$ over incoming angle,
    defined in Eqn.~\ref{eqn:G_per_cosA_2}.
...
\begin{figure}
  \resizebox{\columnwidth}{!}{\input{fig-survival_prob-scat_ih.tex}}
  % Flavor evolution by Yonglin, see emails 5.9.18, 5.16.18
  % Data: $DIR/300-1499kmACC12Step2NE30IH051418m12sc.dat2p
  % Script: $DIR/plot-scat.gp
...
with the dominant effect being a decrease in average energies from the central body,
...
The trajectory starts from 30~km, and moves out radially at
...
At about 1200~km
$e$ neutrinos have almost completely converted to $\mu$ or $\tau$ neutrinos,
while the $e$ antineutrinos have returned back to their original flavor.
...
\item Use the finite-difference hydrodynamics simulation grid to represent
  ...
  input variables to the pseudospectral spacetime simulation grid in order
  to begin ray tracing.
  We have found that though the interpolation of fluid and neutrino variables
  to a lower-resolution pseudospectral grid saves computational memory, it
...
by NASA through the Einstein Postdoctoral Fellowship grant \lstinline{PF4-150122}
awarded by the Chandra X-ray Center,
which is operated by the Smithsonian Astrophysical Observatory for NASA
under contract \lstinline{NAS8-03060},
and through grant \lstinline{80NSSC18K0565} [FF];
through National Science Foundation Grant \lstinline{PHY-1402916} [MDD];
and through the U.S.\ Department of Energy, Office of Science,
Office of Nuclear Physics,
under award number \lstinline{DE-FG02-02ER41216} [GCM].
We thank James P.\ Kneller for his original flavor evolution code base.
