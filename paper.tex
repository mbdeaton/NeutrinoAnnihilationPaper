...
the direction of $\ell_\beta$. We choose $\lambda=\lambda_0$ at $x^\alpha_e$
and $\lambda=\lambda$ at $x^\alpha_o$, as in Fig.~\ref{fig:affine_param}.
\todo{choose a better notation}
...
\begin{figure}
  ...
  \caption{Affine parameterization of a trajectory.}
  \label{fig:affine_param}
\end{figure}

\subsection{The Rendering Equation}
\label{ssec:rendering_eqn}
\todo{type up derivation from notes}
We can integrate Eqn.~\ref{eqn:boltzmann_linear} directly,
\begin{equation}
  \label{eqn:rendering}
  f(\lambda) = f(\lambda_0) e^{-\tau(\lambda,\lambda_0)} +
  \int_{\lambda_0}^\lambda d\lambda'\,e^{-\tau(\lambda,\lambda')}
  \mathscr{K}^*(\lambda') \mathscr{S}^*(\lambda'),
\end{equation}
where the parameterization conventions are depicted in
...
\label{eqn:neutrinosurface}
  f(\lambda) = f^{\rm eq}(\lambda_0),
\end{equation}
where $\tau(\lambda,\lambda_0)=2/3$.

\subsection{Moments of the Distribution Function}
\label{ssec:moments}
\todo{clarify integral differentials}
...
\end{align}
which represent energy density, energy flux, and momentum flux, respectively.
\todo{clarify moment frame and relate to other frames}
...
\todo{derive and type up sources in terms of $J$ and $H^\alpha$
