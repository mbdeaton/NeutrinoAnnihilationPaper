...
\footnote{TODO (Evan): cite others}
...
\footnote{TODO (Evan): discuss mc more clearly}
...
for example neutrino oscillations \footnote{TODO (Gail): cite an example study}
...
\footnote{TODO (Evan, Francois, Gail): cite energy-dependent
neutrino+nucleosynthesis study}
...
\footnote{TODO (Evan): neglecting anything else?}
...
\footnote{TODO (Gail, Yonglin): citation?}
...
\footnote{TODO (Matt, Francois): reasonable assumption? citation?}
...
In Fig.~\ref{fig:survival-scat}
we show the survival probabilities for $\nu_e$ and $\bar{\nu}_e$
for the \emph{scat} treatment. (There is no flavor transformation
in the \emph{noscat} treatment.)
...
\footnote{TODO (Yonglin): define $P_{\nu_\alpha\rightarrow\nu_\alpha}$
  in terms of $S$}
Where $P_{\nu_e\rightarrow\nu_e}$ decreases, for example in
Fig.~\ref{fig:survival-scat} outside of $r\sim400$~km,
a fraction of the original $\nu_e$ content of the beam oscillates into
$\nu_\mu$ and $\nu_\tau$, and a fraction of $\nu_\mu$ and $\nu_\tau$
oscillates into $\nu_e$.
...
\begin{figure}
  ...
  % Flavor evolution by Yonglin, see emails 11.28.17 and 1.31.18
  % Plot: $DIR/brett4sfnew300-600kmACC12Step2NE30NH012518m12sc.pdf
  % w/ DIR=/Users/mbdeaton/Documents/ResearchGraveyard/NuclearAstrophysics/DataAndVis/20170412-self_interaction_potentials_M12_M12-LS220-M1/Run03/RunBA/probabilityPotential-production01
  \caption{Neutrino survival probabilities for the \emph{scat} case.
    In the lower panel we reproduce the self-interaction potential $V_{\nu\nu,0}$
    and matter potential $V_e$ from Fig.~\ref{fig:V_nunu-noscat}.
    \footnote{TODO (Yonglin): change colors and symbols to be consistent with
    text and above plots. Of if you can give me the raw data being plotted, I
    can make the plot.}
    In this case the neutrinos undergo a standard MNR transition
    beginning around $r\sim400$~km,
    with the $e$-flavor neutrinos and antineutrinos partially converting to
    $\nu_x$.
  }
...
oscillations along one toy trajectory exiting the neutrino-dense environment
of the neutron star post-merger configuration (Sec.~\ref{ssec:V_nunu}).
Along that trajectory,
$25^\circ$ from the polar axis, elastic scattering increases the
$\bar{\nu}_e$ relative to the $\nu_e$ self-interaction potential,
introducing a standard matter neutrino
resonance (MNR) where there was none before.
In a similar configuration, but with matter densities lower
by a factor of a few, the effect of elastic scattering would have been to push
the MNR out to larger radii.
...
We caution the reader, however, to avoid drawing general conclusions from this
particular example, since a single astrophysical configuration can present
dramatically different oscillation resonances along test trajectories emerging
at different angles \cite{zhu2016-mnr_nsns_remnant},
and since the MNR is extremely sensitive to a host of parameters.
...
and the National Science Fondation under grant numbers X. \footnote{TODO (all): grant nos?}
