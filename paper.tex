...
\begin{equation}
  ...
  f(t)
  = - \int_{0}^t dt'\,\frac{1}{p^t}e^{-\tau(0,t')}
...
\begin{figure}
  ...
  \caption{Mean free paths
    ...
    below energies at which weak magnetism plays a role.
...
\begin{figure}
  ...
  \caption{Cumulative distribution functions at $\varepsilon=11.25$~MeV
    ...
    The two components of the \emph{scat} solutions ($f_{\rm AE}$ in points,
    and $f_{\rm SE}$ in dashed lines) sum to equal the single component
    of the \emph{noscat} solution ($f_{\rm AE}$ in points connected by lines).
    All solutions terminate when they achieve a total optical depth
    greater than $\tau_term=14$; in the \emph{scat} cases this occurs at an
