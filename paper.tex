...
\begin{align}
  ...
  \label{eqn:source_se}
  \mathscr{S}_{\rm SE}
  &= \frac{\mathscr{K}_{\rm SE}}{\mathscr{K}} \Phi_{\rm SE}.
\end{align}
...
\begin{equation}
  ...
  + \mathscr{K}_{\rm SE}(\Phi_{\rm SE}-f)
\end{equation}
...
toward $\Phi_{\rm SE}$ over an affine lengthscale $\mathscr{K}_{\rm SE}^{-1}$.
...
\begin{align}
  ...
  &= \int_{\lambda_e}^\lambda d\lambda'\,e^{-\tau(\lambda,\lambda')}
  \mathscr{K}_{\rm SE}(\lambda')\Phi_{\rm SE}(\lambda'),
\end{align}
...
\begin{align}
  ...
  \label{eqn:num_fse}
  \frac{df_{\rm SE}}{dt} &=
  \frac{1}{p^t}e^{-\tau}\mathscr{K_{\rm SE}} \,\Phi_{\rm SE}.
\end{align}
...
$f_{\rm bdry}$ is negligible (in practice $e^{-\tau_{\rm term}}=0.001$)
and compute it assuming radiative equilibrium with the matter at $x_e^\alpha$
using Eqn.~\ref{eqn:feq}: $f_{\rm bdry}=f^{\rm eq}\,e^{-\tau_{\rm term}}$.
If the ray terminates off the numerical domain before the terminal optical
depth accumulates, we set $f_{\rm bdry}=0$.
...
\begin{align}
  \ell_i &= q\Omega_i, \\
  \Omega_i &:= (\sin a \cos b,\sin a\sin b,\cos a).
\end{align}
with q and s given by
\begin{align}
  q &= W
\end{align}
And we further specify a rotated frame using two new spherical polar angles
\begin{equation}
  \label{eqn:def_direction_primed_2}
  \Omega_{i'} :=  (\sin A \cos B,\sin A\sin B,\cos A).
\end{equation}
The two frames are related by a standard Euler rotation matrix
\begin{equation}
  \label{eqn:rotation_connection}
  \Omega_i=R^{i'}_i(\theta,\phi)\, \ell_{i'}.
\end{equation}
...
\begin{equation}
  ...
  C_{{\rm SE},i}
  = \varepsilon \chi_{s,i} \Phi_{{\rm SE},i} - \varepsilon \chi_{s,i} f,
\end{equation}
where we have introduced the scattering opacity $\chi_{s,i}$
and the background scattering field $\Phi_{\rm SE}$:
\begin{align}
  \label{eqn:opacity_s}
  \chi_{s,i}(\varepsilon)
  &= \frac{4\pi\varepsilon^3}{(2\pi)^3}R_{{\rm SE},i}^0(\varepsilon), \\
  \label{eqn:background_phi}
  \Phi_{\rm SE,i}(\varepsilon,\ell_\alpha)
...
When we compute $\Phi_{\rm SE}(\varepsilon)$ using $J(\varepsilon)$ and
